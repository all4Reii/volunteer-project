<template>
  <div class="service-records">
    <el-card header="⏱️ 服务时长记录">
      ...
      <el-table :data="records" border stripe v-loading="loading" empty-text="暂无服务记录">
...
export default {
  ...
  setup() {
    ...
    const form = ref({ activityId: 1, activityTitle: '', date: '', hours: 0, remark: '' })

    onMounted(() => store.dispatch('fetchServiceRecords'))

    const loading = computed(() => store.state.loading)
    const records = computed(() => store.state.serviceRecords)
    const totalHours = computed(() => records.value.reduce((s, r) => s + r.hours, 0))
    ...
    const addRecord = async () => {
      ...
      form.value = { activityId: 1, activityTitle: '', date: '', hours: 0, remark: '' }
    }

    return { records, loading, totalHours, dialogVisible, form, addRecord }
  }
}
</script>
